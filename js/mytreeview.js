    ...
    var methods = {
           ...
           return this.each(function(){
              ...
              if ( ! data ) {
                ...
                $this.append("<div class='container_my-table'><table id='"+$this.attr('id')+"table' class='my-table table table-hover'></table></div>");
                if (options.pagination){
                    ...
                    $this.append('<div style="position:relative" class="pag">'+
                        ...
                        '<div style="" class="input-group-btn dropup">'+                    
                        ...
                        '</button>'+
                            ...
                            '</ul>'+
                            '</div>'+
                        '<input intype="num_only" style="margin-left: 15px;width:80px;" data-mask="999" type="text"  class="form-control">'+
                        '</div>'+                       
                        '</div>');
                }
                    ...
                    console.log(dataz);
                        if (data.pagination){
                            if (dataz.totalrows<=data.pagesize){
                                ...
                                var pc = Math.ceil(parseInt(dataz.totalrows)/parseInt(data.pagesize));
                            }
                            data.totalpages= pc;
                            console.log(pc);
                            ...
                            methods.fillData.apply($this,[pid,dataz.data]);
                        }else{
                ...
                $('#'+$this.attr('id')+' .pagination a').click(function () {
                    switch ($(this).attr('type')){
                        case 'i':                           
                            ...
                            data.pagesize=parseInt($('#'+$this.attr('id')+' button.btn.btn-default.dropdown-toggle').text());
                            methods.loadData.apply($this,[$('#'+$this.attr('id')+' .css_tree .label-primary').attr('idgroup')]);
                            break;
                        case 'la':
                            if (data.curpage>1){
            ...
            if (dataz!=null){
                for (var i=0;i<dataz.length;i++){
                    if (dataz[i].isgroup==1 && dataz[i].parentid==z){
                      $('#'+$this.attr('id')+' .css_tree span[idgroup='+z+']').parent().find('ul').append("<li><span class='label label-success' idgroup='"+dataz[i].id+
    ...
    function myTable(myTable,funclicktr){
        ...
        this.fillHeaders = function (data){
            ...
            if(myArray!==null && tableid!==null){
                ...
                $("#"+tableid+" thead tr td:first-child").before("<th><i ></th>"); 
        ...
        this.addRow = function (data,isgroup,id){
            ...
            if(myArray!==null && tableid!==null){
                ...
                if (isgroup==1){
                    ...
                    $("#"+tableid+" tbody .category").last().addClass("selected-row");
                }else{
                    $("#"+tableid+" tbody").append(row);
                    $("#"+tableid+" tbody tr:last-child > td:first-child").before("<td><i style='color:#B85C5C' class='glyphicon glyphicon-file'></td>");
                    $("#"+tableid+" .selected-row").removeClass("selected-row");
                    $("#"+tableid+" tbody tr:last-child").addClass("selected-row animated fadeInDown");                    
                }
...
//                     $("<div id='curredit'></div>").insertBefore("#"+tableid+" thead");
                     ...
                     $("#curredit").width($(this).outerWidth()-1);
                     //$("#curredit").height($(this).height());
                     $("#curredit").height($(this).outerHeight()-3);
